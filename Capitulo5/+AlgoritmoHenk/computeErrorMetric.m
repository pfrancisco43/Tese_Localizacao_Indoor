function [y,UEMean,UECov] = computeErrorMetric(x,Ns,S,measurement,BS)
    ...
    for k=1:N_SP
        for n=1:Ns
            ...
            %rho=measurement(k,1)-x(1)+sqrt(S(1,1));   %maximum distance of SP from the BS
            %thetaSP=measurement(k,2:3)+sqrtm(S(2:3,2:3)); % generate an AOD guess
            ...
            %thetaUE(1)=pi-x(2)-measurement(k,4)+sqrtm(S(4,4));
            %thetaUE(2)=-measurement(k,5)+sqrtm(S(5,5));
    ...
    for n=1:Ns
        ...
        for p1=1:N_SP
            for p2=p1+1:N_SP
                k=k+1;
                [D(n,k),~,pt1,pt2]=AlgoritmoHenk.DistBetween2Segment(xmin(1:3,n,p1),xmax(1:3,n,p1),xmin(1:3,n,p2),xmax(1:3,n,p2));
